...
parser.add_argument('--src_data', required=False, default='source',  help='sec data path')
parser.add_argument('--tgt_data', required=False, default='target',  help='tgt data path')
parser.add_argument('--vgg_model', required=False, default=config.PRE_TRAINED_VGG_DIR, help='pre-trained VGG19 model path')
...
parser.add_argument('--latest_generator_model', default='', help='the latest trained model path')
parser.add_argument('--latest_discriminator_model', default='', help='the latest trained model path')
...
# results save path
if not os.path.isdir(os.path.join(config.TRAIN_OUTPUT_DIR, 'Reconstruction')):
    os.makedirs(os.path.join(config.TRAIN_OUTPUT_DIR, 'Reconstruction'))
if not os.path.isdir(os.path.join(config.TRAIN_OUTPUT_DIR, 'Transfer')):
    os.makedirs(os.path.join(config.TRAIN_OUTPUT_DIR, 'Transfer'))
...
if args.latest_generator_model == '':
    ...
    with open(os.path.join(config.TRAIN_OUTPUT_DIR, 'pre_train_hist.pkl'), 'wb') as f:
    ...
    with torch.no_grad():
        ...
        for n, (x, _) in enumerate(train_loader_src):
            ...
            path = os.path.join(config.TRAIN_OUTPUT_DIR, 'Reconstruction', '_train_recon_' + str(n + 1) + '.png')
        ...
        for n, (x, _) in enumerate(test_loader_src):
            ...
            G_recon = G(x)
            result = torch.cat((x[0], G_recon[0]), 2)
            path = os.path.join(config.TRAIN_OUTPUT_DIR, 'Reconstruction', '_test_recon_' + str(n + 1) + '.png')
            plt.imsave(path, (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
            if n == 4:
                break
else:
    ...
    if epoch % 2 == 1 or epoch == args.train_epoch - 1:
        with torch.no_grad():
            ...
            for n, (x, _) in enumerate(train_loader_src):
                ...
                G_recon = G(x)
                result = torch.cat((x[0], G_recon[0]), 2)
                path = os.path.join(config.TRAIN_OUTPUT_DIR, 'Transfer', str(epoch+1) + '_epoch_' + '_train_' + str(n + 1) + '.png')
                plt.imsave(path, (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
                if n == 4:
                    break
            ...
            for n, (x, _) in enumerate(test_loader_src):
                ...
                G_recon = G(x)
                result = torch.cat((x[0], G_recon[0]), 2)
                path = os.path.join(config.TRAIN_OUTPUT_DIR, 'Transfer', str(epoch+1) + '_epoch_' + '_test_' + str(n + 1) + '.png')
                plt.imsave(path, (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
                if n == 4:
                    break

            torch.save(G.state_dict(), os.path.join(config.TRAIN_OUTPUT_DIR, 'generator_latest.pkl'))
            torch.save(D.state_dict(), os.path.join(config.TRAIN_OUTPUT_DIR, 'discriminator_latest.pkl'))
...
torch.save(G.state_dict(), os.path.join(config.TRAIN_OUTPUT_DIR,  'generator_param.pkl'))
torch.save(D.state_dict(), os.path.join(config.TRAIN_OUTPUT_DIR,  'discriminator_param.pkl'))
with open(os.path.join(config.TRAIN_OUTPUT_DIR,  'train_hist.pkl'), 'wb') as f:
